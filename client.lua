...
--TriggerEvent('chat:addSuggestion', '/Ep', 'car features', {{name="pilot|dance|mark", help="Enable autopilot, Dance or mark your current vehicle "}})
RegisterCommand("ep", function(source, args)
	if(args[1] == "mark") then
		if(IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), false), -1) == GetPlayerPed(-1)) then
			ep = GetVehiclePedIsIn(GetPlayerPed(-1), false)
			...
			tesla_blip = AddBlipForEntity(ep)
		...
		elseif(ep) then
			if(args[1] == "pilot") then
				if(tesla_pilot) then
					...
					tesla_pilot = false
					tesla_pilot_ped = nil
					SetVehicleEngineOn(ep, false, false, false)
					minimap("Auto-Pilot canceled.")
				else
					RequestModel(225514697)
					while not HasModelLoaded(225514697) do
					...
					minimap("Auto-Pilot activated.")
					tesla_pilot = true
					tesla_pilot_ped = CreatePed(0, 225514697, GetEntityCoords(ep)["x"], GetEntityCoords(ep)["y"], GetEntityCoords(ep)["z"], 0.0, false, true)
					SetPedIntoVehicle(tesla_pilot_ped, ep, -1)
					...
					TaskVehicleDriveToCoord(tesla_pilot_ped, ep, player_coords.x, player_coords.y, player_coords.z, 100.0, 1.0, GetHashKey(ep), 786603, 1.0, 1)
					Citizen.CreateThread(function()
						while tesla_pilot do
							Wait(100)
							if(GetDistanceBetweenCoords(GetEntityCoords(ep)["x"], GetEntityCoords(ep)["y"], GetEntityCoords(ep)["z"], player_coords.x, player_coords.y, player_coords.z, 0) < 10.0) then
								while GetEntitySpeed(ep) - 1.0 > 0.0 do
									SetVehicleForwardSpeed(ep, GetEntitySpeed(ep) - 1.0)
									Wait(100)
								end
								tesla_pilot = false
								--DeletePed(tesla_pilot_ped)
				...
				if(tesla_dance) then
					...
					Citizen.CreateThread(function()
						while tesla_dance do
							Wait(100)
							SetVehicleDoorOpen(ep, math.random(0, 6), false, false)
							SetVehicleDoorShut(ep, math.random(0, 6), false)
						end
					end)
				end
			end
